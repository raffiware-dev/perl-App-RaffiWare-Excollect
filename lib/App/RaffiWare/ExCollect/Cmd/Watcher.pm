...
sub _build_daemon {
  ...
  return Proc::Daemon->new( work_dir => $self->cmd_dir, pid_file => $self->cmd_dir .'/watcher.pid' );
}
...
sub do_start {
  ...
  if ( $self->no_daemonize ) {
     ...
     open( my $FH_PIDFILE, "+>", $pid_file ) ||
                    die "Can not open pidfile (pid_file => '$pid_file'): $!";
  ...
  # TODO  init timer for replay requests
...
sub queue_job {
  ...
  $self->worker_pool->( job      => $job->job_id,
                        cmd_dir  => $job->cmd_dir,
                        cfg_file => $job->cfg_file,
                        debug    => $self->debug,
                        sub { DEBUG("Worker returned @_") } );
  $job->unlock();
...
sub _build_job_fetch_timer {
  ...
  # TODO introduce random sleep before API call
  return AE::timer 0, $self->job_poll_interval,
             sub {
                  DEBUG("Checking for new jobs");

                  my $job_msg = $self->api->get_next_job() or return;

                  if ( $self->load_job($job_msg->{data}->{id}) ) {
                       WARNING(sprintf('Job %s already fetch', $job_msg->{data}->{id} ));
                       return 
                  }

                  INFO(sprintf("Got new job %s", $job_msg->{data}->{id}));
                  DEBUG(Dumper($job_msg->{data}));

                  my $job = $self->init_job($job_msg->{data});
...
sub init_worker_pool {
  ...
  # On installed systems this will be a 
  # self container PAR executable.
  my $anyevent_fork_bin = File::Which::which("anyevent-fork") or die("anyevent-fork not in PATH");
