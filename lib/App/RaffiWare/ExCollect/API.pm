...
sub get_command_user_key {
  ...
  if (!$key_data) {
    my $resp = $self->signed_request( get => $self->uri_base ."/jobs/$job_id/command_instance/command/all_users/$user_id/public_key" );
...
sub register_host {
  ...
  my $resp = $self->request( post => "/excollect_api/hosts/register",
                             body => { 
...
sub get_next_job {
  ...
  my $resp = $self->signed_request( post => $self->uri_base .'/jobs/get_next',
...
sub update_job {
  ...
  return $self->signed_request( patch => $self->uri_base ."/jobs/$job_id",
                                body  => $update ); 
}
...
sub add_job_log {
  ...
  my $resp = $self->signed_request( post => $self->uri_base ."/jobs/$job_id/job_logs",
                                    body => [$log] ); 
