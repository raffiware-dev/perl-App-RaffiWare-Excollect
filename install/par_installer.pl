...
use lib  $ENV{EXC_PERL_ROOT}.'/lib/perl5'; 
use local::lib $ENV{EXC_PERL_ROOT};

use Data::Dumper;
my $BASE  =  $ENV{EXC_PERL_ROOT};
require Config; 
#$ENV{PERL_INSTALL_ROOT} = $ENV{EXC_PERL_ROOT};  
...
# Tell PAR::Dist how to install to our local lib.
require Config;

my $dir     = File::Spec->curdir;
my $bin     = "$BASE/bin";
my $lib     = "$BASE/lib/perl5"; 
my $arch    = $Config::Config{archname};
my $archlib = "$BASE/lib/perl5/$arch";  
my $man1    = "$BASE/man/man1";
my $man3    = "$BASE/man/man3"; 
my $name    = 'exc_install';
...
my $targets = {
     read                   => $archlib ."/auto/$name/.packlist", 
     write                  => $archlib ."/auto/$name/.packlist", 
     $sources{inst_lib}     => $lib,
     $sources{inst_archlib} => $archlib,
     $sources{inst_bin}     => $bin,
     $sources{inst_script}  => $bin,
     $sources{inst_man1dir} => $man1,
     $sources{inst_man3dir} => $man3, 
};

PAR::Dist::install_par( dist => shift(@ARGV), custom_targets => $targets );
